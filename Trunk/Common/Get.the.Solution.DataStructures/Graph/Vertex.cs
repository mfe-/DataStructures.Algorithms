...
namespace Get.the.Solution.DataStructure
{
        ...
        where W : IComparable<W>
    {
        ...
        public virtual void RemoveEdge(IVertex<W,D> U, bool Undirected)
        {
           // IEdge<W, IVertex<W>> edge = this.Edges.Where(a => a.U.Equals(this) && a.V.Equals(U)).FirstOrDefault<IEdge<W>>();

            if (Undirected.Equals(false))
            {
                //IEdge<W, IVertex<W>> edged = edge.Left.Edges.Where(a => a.U.Equals(edge.V) && a.V.Equals(this) && a.Weight.Equals(edge.Weight)).FirstOrDefault<IEdge<W>>();

             //   edge.V.Edges.ToList<IEdge<W, IVertex<W>>>().Remove(edged);
            }
           // this.Edges.ToList<IEdge<W, IVertex<W>>>().Remove(edge);
        }
        ...
        {
            if (!obj.GetType().Equals(typeof(IVertex<W,D>))) return false;

            return this._Guid.Equals((obj as Vertex<W,D>)._Guid);
        }
        /// <summary>
